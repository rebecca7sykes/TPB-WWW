...
$fc_pref = $GLOBALS['fc_config']['db']['pref'];
	...
	//All checks are over, start xml reply
			...
			if($rs = $stmt->process($this->id, $this->session_inst)) {//changed on 090706 for chat instances
				...
				while($rec = $rs->next()) {
				  if ($GLOBALS['fc_config']['combineCMS']) {
  				  $GLOBALS['fc_config']['CMSsystem'] = (1000000 <= $rec['userid']) ? 'statelessCMS' : $GLOBALS['fc_config']['previousCMSsystem'];
 				  }
			...
			$GLOBALS['fc_config']['CMSsystem'] = $GLOBALS['fc_config']['previousCMSsystem'];
