	...
	class DefaultCMS {
		...
		function DefaultCMS()
		{
			...
			$this->constArr = array(
					  ...
					  'spy_fld'     =>'roles',
					  'spy_value'   =>'4',
					  'profile_path'=>'http://localhost/inet/FlashChat5/chat/profile.php',
					  'profile_arg' =>'?user={$id}',
					  'admin_fld'=>'roles',
					  'admin_value'=>'2',
					  'logoff'		=>'false',
        ...
        $usr['roles'] = $usr[$this->constArr['admin_fld']] == $this->constArr['admin_value'] ? ROLE_ADMIN :($usr[$this->constArr['moderator_fld']] == $this->constArr['moderator_value'] ? ROLE_MODERATOR : ($usr[$this->constArr['spy_fld']] == $this->constArr['spy_value'] ? ROLE_SPY : ROLE_USER));
				return $usr;
			}
			else
			{
		...
		function getUsers()
		{
			...
			foreach($users as $k => $v)
			{
			  ...
			  $users['roles'] = $usr[$this->constArr['admin_fld']] == $this->constArr['admin_value'] ? ROLE_ADMIN :($usr[$this->constArr['moderator_fld']] == $this->constArr['moderator_value'] ? ROLE_MODERATOR : ($usr[$this->constArr['spy_fld']] == $this->constArr['spy_value'] ? ROLE_SPY : ROLE_USER));
		...
		function getUserProfile($userid)
		{
			...
			extract($user);
			return "{$this->constArr['profile_path']}{$this->constArr['profile_arg']}{$userid}";
		}
		//-----------------------------------------------------------------------------------------
